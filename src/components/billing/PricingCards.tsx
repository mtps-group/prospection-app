...
export function PricingCards() {
  ...
  return (
    <div ref={outerRef} className="relative px-8">
      ...
      {/* ── Flèche droite — retour vers Agence ── */}
      <button
      ...
      <div className="overflow-hidden">
        ...
        >
          ...
          <div style={{ width: cw, minWidth: cw, flexShrink: 0 }} className="flex flex-col">
            <Card3D
              intensity={6}
              className="relative rounded-2xl bg-white border border-gray-200 p-8 flex flex-col overflow-hidden h-full"
              style={{ boxShadow: '0 4px 6px -1px rgba(0,0,0,0.07), 0 10px 30px -5px rgba(0,0,0,0.1)', minHeight: 580 }}
            >
          ...
          <div style={{ width: cw, minWidth: cw, flexShrink: 0 }} className="flex flex-col pt-6">
            ...
            <Card3D
              intensity={8}
              className="relative rounded-2xl bg-gradient-to-br from-primary via-indigo-600 to-purple-600 p-8 flex flex-col overflow-hidden flex-1"
              style={{ boxShadow: '0 8px 16px -2px rgba(99,102,241,0.35), 0 20px 50px -8px rgba(99,102,241,0.4), inset 0 1px 0 rgba(255,255,255,0.15)', minHeight: 580 }}
            >
              ...
              <div className="relative">
                {currentPlan === 'premium' ? (
                  <button onClick={handleManageSubscription} className="block w-full text-center rounded-xl bg-white/20 border border-white/30 py-3.5 font-semibold text-white hover:bg-white/30 transition-colors text-sm">
                    ✓ Plan actuel · Gérer
                  </button>
          ...
          <div style={{ width: cw, minWidth: cw, flexShrink: 0 }} className="flex flex-col">
            <Card3D
              ...
              style={{ background: 'linear-gradient(145deg, #0f0f1a 0%, #1a1a2e 50%, #16213e 100%)', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.3), 0 10px 30px -5px rgba(0,0,0,0.4), 0 0 0 1px rgba(251,191,36,0.15)', minHeight: 580 }}
            >
              ...
              <div className="relative">
                {currentPlan === 'ultra' ? (
                  <button onClick={handleManageSubscription} className="block w-full text-center rounded-xl border border-amber-400/30 bg-amber-400/10 py-3.5 font-semibold text-amber-400 hover:bg-amber-400/20 transition-colors text-sm">
                    ✓ Plan actuel · Gérer
                  </button>
                ) : isPaidHigher('ultra') ? (
                  <div className="block text-center rounded-xl bg-white/10 py-3.5 font-semibold text-white/50 text-sm">Plan inférieur</div>
          ...
          <div style={{ width: cw, minWidth: cw, flexShrink: 0 }} className="flex flex-col">
            <Card3D
              intensity={6}
              className="relative rounded-2xl p-8 flex flex-col overflow-hidden h-full"
              style={{ background: 'linear-gradient(145deg, #0d0015 0%, #1a0030 50%, #0d001a 100%)', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.4), 0 10px 30px -5px rgba(0,0,0,0.5), 0 0 0 1px rgba(167,139,250,0.2)', minHeight: 580 }}
            >
              ...
              <div className="relative">
                {currentPlan === 'agence' ? (
                  <button onClick={handleManageSubscription} className="block w-full text-center rounded-xl border border-violet-400/30 bg-violet-400/10 py-3.5 font-semibold text-violet-400 hover:bg-violet-400/20 transition-colors text-sm">
                    ✓ Plan actuel · Gérer
                  </button>
                ) : (
      ...
      <div className="flex justify-center gap-2 mt-6">
        <button
          onClick={() => setShowGratuit(true)}
          className={`h-2 rounded-full transition-all duration-300 ${showGratuit ? 'w-6 bg-primary' : 'w-2 bg-gray-300 hover:bg-gray-400'}`}
          aria-label="Voir plan Gratuit"
        />
        <button
          onClick={() => setShowGratuit(false)}
          className={`h-2 rounded-full transition-all duration-300 ${!showGratuit ? 'w-6 bg-primary' : 'w-2 bg-gray-300 hover:bg-gray-400'}`}
          aria-label="Voir plans payants"
        />
      </div>
