...
export function PricingCards() {
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4 items-stretch" style={{ perspective: '1200px' }}>
      ...
      >
        ...
        <div className="relative mb-3">
          ...
          <p className="text-xs text-text-muted">Pour tester sans risque</p>
        </div>
        <div className="relative mb-3">
          <span className="text-3xl font-black text-text">0€</span>
          <span className="text-text-muted ml-1 text-xs">/ mois</span>
        </div>
        ...
        <div className="relative">
          {currentPlan === 'free' ? (
            <div className="block text-center rounded-xl border-2 border-primary/30 bg-primary-light py-2.5 font-semibold text-primary text-sm">
              ✓ Plan actuel
            </div>
          ) : (
            <div className="block text-center rounded-xl border-2 border-gray-200 py-2.5 font-semibold text-text-muted text-sm">
              Plan gratuit
            </div>
          )}
        </div>
      ...
      <div className="relative pt-5 h-full flex flex-col">
        ...
        >
          ...
          {/* mt-5 pour laisser de la place au badge */}
          ...
          <div className="relative">
            {currentPlan === 'premium' ? (
              <button
                ...
                className="block w-full text-center rounded-xl bg-white/20 border border-white/30 py-2.5 font-semibold text-white hover:bg-white/30 transition-colors text-sm"
              >
                ✓ Plan actuel · Gérer
              </button>
            ) : isPaidHigher('premium') ? (
              <div className="block text-center rounded-xl bg-white/10 py-2.5 font-semibold text-white/50 text-sm">
                Plan inférieur
              </div>
            ) : (
              <button
                ...
                className="block w-full text-center rounded-xl bg-white py-2.5 font-bold text-primary hover:bg-gray-50 transition-all text-sm disabled:opacity-70 shadow-lg shadow-black/10"
              >
                {loadingPlan === 'premium' ? (
      ...
      >
        ...
        <div className="relative">
          {currentPlan === 'ultra' ? (
            <button
              onClick={handleManageSubscription}
              className="block w-full text-center rounded-xl border border-amber-400/30 bg-amber-400/10 py-2.5 font-semibold text-amber-400 hover:bg-amber-400/20 transition-colors text-sm"
            >
              ✓ Plan actuel · Gérer
            </button>
          ) : isPaidHigher('ultra') ? (
            ...
            <button
              ...
              className="block w-full text-center rounded-xl py-2.5 font-bold text-gray-900 hover:opacity-90 transition-all disabled:opacity-70 shadow-lg shadow-amber-400/20 text-sm"
      ...
      >
        ...
        <div className="relative">
          {currentPlan === 'agence' ? (
            <button
              onClick={handleManageSubscription}
              className="block w-full text-center rounded-xl border border-violet-400/30 bg-violet-400/10 py-2.5 font-semibold text-violet-400 hover:bg-violet-400/20 transition-colors text-sm"
            >
              ✓ Plan actuel · Gérer
            </button>
          ) : (
            <button
              ...
              className="block w-full text-center rounded-xl py-2.5 font-bold text-white hover:opacity-90 transition-all disabled:opacity-70 shadow-lg shadow-violet-500/30 text-sm"
